...
For semantic search over a knowledge base — not just conversation history — see the [RAG guide](/guides/rag). The RAG layer uses Qdrant as a vector store and exposes `rag.NewQdrantRetriever` and `rag.NewQdrantIngestor` built on the same `qdrant.Store` used for memory.
