...
The LLM can call `retriever_tool` with `{"query": "..."}` to fetch relevant context on demand. For automatic injection on every turn, use `chainforge.WithRetriever` instead. See the [RAG guide](/guides/rag).
